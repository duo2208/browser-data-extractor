...
- 실제 응답에서 확인한 `body.profile.profileNo`를 우선 추출하고, 스키마 변경에 대비해 재귀 탐색을 fallback으로 사용합니다.
- Network → URL → DOM 순으로 값을 탐색해 특정 화면 구조에 대한 의존도를 낮췄습니다.
- 실행 환경과 자격증명을 분리하고, 실패 원인을 설정 / 로그인 / 추출 단계로 구분합니다.
...
정상 로그인 세션에서 확인한 내용과 구현 세부 사항입니다.
...
검증 과정에서 로그인 실패 원인을 화면 문구만으로 판단하지 않고, 실제 요청 발생 여부와 서버 응답을 함께 확인했습니다.

- CJ ONE 화면의 `계정 잠금 안내`는 로그인 결과가 아닌 상시 노출 문구임을 확인했습니다.
- `code: 1031` 응답을 통해 실제 인증 실패를 별도로 구분했습니다.
- 일부 실행에서는 인증 gateway 요청 자체가 발생하지 않았으며, `grecaptcha is not defined` 오류를 확인해 자격증명 실패와 클라이언트 측 인증 초기화 실패를 구분했습니다.
- 유효한 자격증명 적용 후 로그인과 `profileNo` 추출을 3회 반복해 동일한 결과가 반환되는 것을 확인했습니다.
